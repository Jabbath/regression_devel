...
class Fate_Lasso():
   ...
   #################################################
    ...
    def train_type_at_hpf(self, cell_type, train, test, alpha, positive=False):    
        ...
        #Train the model
        reg = linear_model.ElasticNet(alpha=alpha, l1_ratio=0.001)
...
#                 knee = kn.knee
                knee = max(scores_filt)
                knees.append(alphas_filt[scores_filt.index(knee)])
                
                #Plot the knee as a red dot
                plt.scatter(num_genes_arr_filt[scores_filt.index(knee)], knee, color='red')
                print('Knee found at ', alphas_filt[scores_filt.index(knee)])
            else:
